...
set_input_delay -clock clk 1.100 [get_ports valid_in]

# Tells the tool: "The reset (rst) signal will arrive 5.0 ns AFTER the clock edge."
set_input_delay -clock clk 0.500 [get_ports rst]

# This 'foreach' loop is a shortcut. Instead of writing 32 lines of code for our 16-bit 
# sw bus, we loop through the numbers 0 to 3.
foreach bit {0 1 2 3} {
    # Applies a 0.5 ns input delay to every single bit (0 through 3) of the stream_sw_in bus.
    set_input_delay -clock clk 0.500 [get_ports "stream_sw_in[$bit]"]
...
############################
# Another loop, this time for our 4-bit output bus.
foreach bit {0 1 2 3} {
    ...
    set_output_delay -clock clk 0.500 [get_ports "stream_out[$bit]"]
}
# Applies the same 3.0 ns output requirement to the valid_out signal.
set_output_delay -clock clk 0.500 [get_ports valid_out]
